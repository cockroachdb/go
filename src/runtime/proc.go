...
func maybeYield() {
	...
	if p := gp.m.p.ptr(); p.runqhead != p.runqtail || p.runnext != 0 {
		...
		// global yieldq so the other stops seeing it in the P's runq and
		// yielding to it.
		//
		...
		// instead. We can safely overwrite the normal packed time+counter value in
		// yieldchecks here since we're actively yielding and don't need the counter
		// to make that decision. The sentinel value 1 is also a valid packed value
		// (count=1, time epoch=0), so the time-check logic below handles it
		// correctly: it will see the zero upper bits as a stale epoch and reset
		// yieldchecks to the current time, clearing our sentinel.
		if gp.yieldchecks == 1 {
			yieldPark()
			return
		}
	...
	// maybeYield call. Since our goal is to bound how long work waits, what we
	// instead can do is perform these more expensive checks when enough time has
	...
	// instead, while still below the typical latencies of e.g. network services
	// and far below the default non-cooperative preemption interval.
	//
	...
	// counter is of the form 2^k-1 (i.e. 1, 3, 7, 15, ...). NB: We choose -1 here
	...
	// rate of 1 clock check per 1k calls if called in a very tight loop.
	//
	// Note: Wrapping of the quantized time is possible (2^21 * 0.25ms = ~9min)
	// but given that Yield() needs to be called so much more frequently than that
	...
	if count := gp.yieldchecks & yieldCountMask; (count & (count + 1)) == 0 {
		...
		if now != prev {
			gp.yieldchecks = now
			// Check runqs of all Ps; if we find anything, park to free this P to steal.
			for i := range allp {
